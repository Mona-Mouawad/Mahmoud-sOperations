...
class AnalysisModel with ChangeNotifier{
  ...
  Future getAnalysisList(String searchName , ) async {
    analysisList = [];
  ...
  deleteAnalysisById(int analysisId ,String searchName , ) async
  {
    database.rawDelete('DELETE FROM Analysis  WHERE AnalysisId =?',
        [analysisId]).then((value) => getAnalysisList(searchName));
  }

  editAnalysisByID(int analysisId ,String analysisName, dynamic price ,String searchName ) async
  {
    log("editAnalysisByID   analysisName  $analysisName   analysisId  $analysisId  ");
    await  database.rawUpdate('UPDATE Analysis SET AnalysisName = ?, AnalysisPrice = ? WHERE AnalysisId = ?',
  ...
  Future<void> insertNewAnalysis(String name, dynamic price ,String searchName ,) async
  {
    await database.transaction((txn) =>
        txn.rawInsert('INSERT INTO Analysis (AnalysisName , AnalysisPrice) VALUES("$name",$price)')
            .then((value) {
  ...
  getAnalysisName(String name) {
    log("analysisList length   $name ${analysisList.length}");
      analysisNameList = analysisList.where((element) => element.analysisName!.toUpperCase()
