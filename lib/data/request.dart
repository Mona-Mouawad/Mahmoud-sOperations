...
class InsertTotDB {
  static Future addRequest(context,
      ...
      required List<AnalysisData> analysisSelectedList}) async {
    RepresentativesReportData reportData =
        Provider.of<RepresentativesReportData>(context, listen: false);
    ...
    for (var element in analysisSelectedList) {
      database
          .transaction((txn) => txn.rawInsert(
              'INSERT INTO report(date,analysisQuantity,Labs_Id,Representatives_Id,AnalysisId,AnalysisPrice,PatientName) '
              'VALUES(${date.replaceAll("-", "")},${element.analysisQuantity},$labsId,$representativesId,${element.analysisId},${element.analysisQuantity * element.analysisPrice!},$patientName)'))
          .then((value) async {
        AppConstants.myToast(
            context: context,
            text: AppStrings.entryDone,
            state: ToastStates.success);
      });
    }
    await database
        .rawQuery('SELECT * FROM RepresentativesDayCost '
      ...
      if (value.isEmpty) {
        ...
        reportData.transValue =
            (reportData.transValue == 0) ? 4.0 : reportData.transValue + 4;
    ...
    for (var element in analysisSelectedList) {
      database
          .transaction((txn) => txn.rawInsert(
              'INSERT INTO report(date,analysisQuantity,Labs_Id,Representatives_Id,AnalysisId,AnalysisPrice,PatientName) '
              'VALUES(${date.replaceAll("-", "")},-${element.analysisQuantity},$labsId,$representativesId,${element.analysisId},-${element.analysisQuantity * element.analysisPrice!},$patientName)'))
          .then((value) => AppConstants.myToast(
              context: context,
              text: AppStrings.entryDone,
              state: ToastStates.success));
    }
